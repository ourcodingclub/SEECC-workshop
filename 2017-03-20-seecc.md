...
INSERT TABLE OF PROS AND CONS OF EACH? Then go through examples from each category. - To make linear models for each population of popsize~time
...
Now that we have our finished dataset with all this extra information, let's save it just in case R crashes or your laptop runs out of battery:

```r
save(LPI_long_models, file = "LPI_long_models.RData")
```

...
and then make some summary statistics and adjustments to the data to be stored in the main data frame, to use later:
...
Using our data set, we want to create linear models which demonstrate the abundance trend over time, for each of our populations, then extract model coefficients and other useful stuff to a data frame. Do this using pipes:
...
```r
save(LPI_models_pipes, file = "LPI_long_models.RData")
```

...
LPI_list_lm <- lapply(LPI_long_list, function(x) lm(pop ~ year, data = x))
...
Once you've installed the package by running `install.packages("ggExtra")`, you can select the `ggplot2` code, click on `ggplot2 Marginal Histograms` from the Addin menu and build your plot. Copy the code for the plot and add it to your script afterwards.

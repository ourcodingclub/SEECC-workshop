...
Now that our dataset is *tidy* we can start to get it ready for our analysis. This is quite a messy data frame with data from lots of different sources so to help answer our question (WHAT IS THE QUESTION?) we should create some new variables and filter on them to remove bad data.

...
The variable names are a mixture of upper and lower case letters and use `.` to mark the end of a word. For the sake of consistency and making things easier to read, let's replace any `.` with `_` using `gsub()`and make everything lower case using `tolower()`:
...
Believe it or not, [Bååth (2012) wrote an entire paper on naming conventions in R](https://journal.r-project.org/archive/2012-2/RJournal_2012-2_Baaaath.pdf)!
...
Make a column with a unique population identifier:
